...
export function CarForm({
  ...
  function getCarContext() {
    ...
    if (!brandId || !categoryId || !model || !year) return null;

    const brandName = brands.find((b) => b.id === brandId)?.name;
    const categoryName = categories.find((c) => c.id === categoryId)?.name;
    if (!brandName || !categoryName) return null;
    ...
    return {
      ...
      topSpeed: num("top_speed_kmh"),
      acceleration: num("acceleration_0_100"),
      fuelType: (fd.get("fuel_type") as string) || undefined,
      transmission: (fd.get("transmission") as string) || undefined,
      exteriorColor: str("exterior_color"),
      interiorColor: str("interior_color"),
    };
  }
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div>
            ...
            <select
  name="status"
  defaultValue={car?.status ?? "musait"}
  className={inputClass}
>
  ...
  <option value="satildi">Satıldı</option>
          ...
          <div>
            ...
            <select
              name="condition"
              defaultValue={car?.condition ?? "ikinci_el"}
              className={inputClass}
            >
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>
            ...
            <select
              name="fuel_type"
              defaultValue={car?.fuel_type ?? "benzin"}
              required
              className={inputClass}
            >
          ...
          <div>
            ...
            <select
              name="transmission"
              defaultValue={car?.transmission ?? "otomatik"}
              required
              className={inputClass}
            >
          ...
          <div>
            ...
            <input
              ...
              defaultValue={car?.engine_displacement ?? ""}
              className={inputClass}
            />
          </div>
          <div>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-3 cursor-pointer text-sm">
              <input
                ...
                defaultChecked={car?.has_service_history ?? false}
                className="accent-accent w-4 h-4"
              />
              Servis Geçmişi
            </label>
            <label className="flex items-center gap-3 cursor-pointer text-sm">
              <input
                ...
                defaultChecked={car?.has_damage_record ?? false}
                className="accent-accent w-4 h-4"
              />
              Hasar Kaydı
            </label>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xs tracking-[0.3em] uppercase text-accent">
            Açıklama
          </h2>
          <AIDescriptionButton
            getContext={getCarContext}
            onGenerated={(text) => {
      ...
      <div className="border-t border-border pt-8">
        {state.error && (
          <div className="text-xs text-accent border-l-2 border-accent pl-3 py-2 mb-4">
            {state.error}
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={pending}
          className="px-8 py-3 bg-foreground text-background hover:bg-accent transition-colors text-xs tracking-[0.3em] uppercase disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {pending
            ? "Kaydediliyor..."
            : isEdit
